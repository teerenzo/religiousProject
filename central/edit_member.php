    ...
    if (empty($fname) || empty($email) || empty($phone) || empty($birth) || empty($sname) || empty($id)) {
        ...
        if ($id != $data_ID) {
            if (mysqli_num_rows($check_id) > 0) {
                ...
                $query = $conn->query("UPDATE believers SET fname='$fname',lname ='$sname' ,father='$father',mother='$mother',id_number='$id',date_of_birth='$birth',email='$email',tel='$phone',central_name='$cent_name',sucursal_name='$suc',basic_name='$basic',parish_id='$parish_id' WHERE believer_ID='$cid'");
                if ($query) {
