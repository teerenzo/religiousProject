...
$get_admin = $conn->query("SELECT * FROM users WHERE ((user_id='" . $_SESSION["id"] . "')AND(user_type='central'))");
...
if (isset($_POST["change-info"])) {
    ...
    $email = $_POST["email"];
    $phone = $_POST["phone"];
    $getPass = $conn->query("SELECT * FROM users WHERE ((user_id='" . $_SESSION["id"] . "')AND(user_type='central'))");
    $get_userInfo = mysqli_fetch_array($getPass);
    $current_pas = $get_userInfo["password"];
    //--------------- getting db email -------------------
    $getEmail = $conn->query("SELECT * FROM centrals WHERE id='" . $_SESSION["id"] . "'");
    ...
    if (empty($currpass)) {
        ...
        if ($currpass != $current_pas) {
            ...
            $check_email = $conn->query("SELECT * FROM centrals WHERE email ='$email'");
            if (($get_userInfo["username"] != $username)) {
                if (mysqli_num_rows($check_username) > 0) {
                    ...
                    $email_query = $conn->query("UPDATE centrals SET email='$email',phone='$phone' WHERE id='" . $_SESSION["id"] . "'");
                ...
                if (mysqli_num_rows($check_email) > 0) {
                    ...
                    $query = $conn->query("UPDATE users SET username='$username',email='$email',phone='$phone' WHERE id='" . $_SESSION["id"] . "'");
                    $email_query = $conn->query("UPDATE centrals SET email='$email',phone='$phone' WHERE ((user_id='" . $_SESSION["id"] . "')AND(user_type='central'))");
                    echo '<script>alert("Profile updated successfully.");</script>';
                    echo "<script>window.location.href='profile';</script>";
                }
            } else {
...
if (isset($_POST["save-pass"])) {
    ...
    if (empty($old_pass)) {
        ...
        if ($old_pass != $current_pas) {
            ...
            $change_pass = $conn->query("UPDATE users SET password='$new_pass' WHERE ((user_id='" . $_SESSION["id"] . "')AND(user_type='central'))");
            if ($change_pass) {
