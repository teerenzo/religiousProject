...
<?php 
    if(isset($_POST["submit"])){
        ...
        $check_exist = $conn->query("SELECT * FROM sacrament_issuing WHERE (((parish_id='".$_SESSION['id']."')AND(believer_id='$log_id')AND(issue='Sustenance')))");
        $check_church = $conn->query("SELECT * FROM believers WHERE ((church_id='$ch_id')AND(log_id='$log_id'))");
        if(mysqli_num_rows($check_church) > 0){
            ...
            if(empty($_POST["sys_id"])){
                ...
                if(mysqli_num_rows($check_exist) > 0){
                    echo "<script>alert('This person of this id ".basename($log_id)." is alredy have sustenance sacrament.');</script>";
                }else{
                    $query_system_ins = $conn->query("INSERT INTO sacrament_issuing(issue,parish_id,believer_id) VALUES('Sustenance','".$_SESSION['id']."','$log_id')");
                    if($query_system_ins){
